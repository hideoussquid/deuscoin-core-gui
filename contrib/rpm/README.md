...
The RPM spec file provided here is for Deuscoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://deuscoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Deuscoin git repository](https://github.com/deuscoin/deuscoin) but are not part of
...
Currently only `Source10` is used, for the example `deuscoin.conf` file.
...
in the deuscoin git repository. At present only an SVG version of the Deuscoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the deuscoin
...
needed when 0.12.1 is released, a proper fix is already in the Deuscoin git
master branch.
...
idea to build Deuscoin Core against a static version of that library compiled
...
The `deuscoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `deuscoin-server` package. At this point, most RPM based
...
The `deuscoin-server` package should have SELinux support. How to properly do
...
Deuscoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/deuscoin/). He has been
packaging Deuscoin for Fedora at least since 2012.
...
4. I renamed the `deuscoin` package that contains the Qt GUI to `deuscoin-core` as
that appears to be how the general population refers to it, in contrast to
`deuscoin-xt` or `deuscoin-classic`. I wanted to make sure the general population
