...
###   This script attempts to download the signature file SHA256SUMS.asc from deuscoin.org
...
WORKINGDIR="/tmp/deuscoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://deuscoin.org/bin/"
VERSIONPREFIX="deuscoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'deuscoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Deuscoin Core binary release signing key installed?"
   fi
