...
const char* BIP71_MIMETYPE_PAYMENT = "application/deuscoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/deuscoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/deuscoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the deuscoin: URI contains a payment request, we are not able to detect the
...
//
// OSX-specific way of handling deuscoin: URIs and PaymentRequest mime types.
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom deuscoin addresses are not supported
        ...
        // Deuscoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
